...
static bool g_above100 = false;  // current CH area >= 100 state
...
// sleeps until signaled, prints a message when the CH area crosses the 100 threshold
void* monitorThread(void*) {
    ...
    while (g_running) {
        ...
        if (cur && !last)
            puts("At Least 100 units belongs to CH");
        else if (!cur && last)
            puts("At Least 100 units no longer belongs to CH");
...
void* handleClient(int fd) {
    ...
    while (true) {
        ...
        if (input.rfind("Newgraph ", 0) == 0) {
            ...
            // notify monitor thread if threshold state changed
            pthread_mutex_lock(&g_mutex);
            bool nowAbove = (area >= 100.0);
            bool changed  = (nowAbove != g_above100);
            g_above100    = nowAbove;
            if (changed) pthread_cond_signal(&g_cond);
            pthread_mutex_unlock(&g_mutex);
...
int main() {
    ...
    pthread_t monTid;
    pthread_create(&monTid, nullptr, monitorThread, nullptr);
